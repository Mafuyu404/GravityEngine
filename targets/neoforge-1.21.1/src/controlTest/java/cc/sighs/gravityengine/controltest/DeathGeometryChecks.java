...
final class DeathGeometryChecks {
    ...
    static void run(ServerLevel level) {
        ...
        try {
            ...
            for(int mode=0;mode<3;mode++) {
                var actor=cow(level); int before=actor.sizeEvents;
                int selected=mode;
                actor.callback=()->{
                    actor.throwSize=selected==2;
                    actor.refreshDimensions();
                    coherentGeometry(actor);
                    if(selected==1) throw new FixtureFailure();
                };
                boolean threw=false;
                try { actor.move(MoverType.SELF,new Vec3(.01,-.1,0)); }
                catch(FixtureFailure expected) { threw=true; }
                check(threw==(mode>=1),"callback exception propagated");
            ...
            var failedStandalone = cow(level);
            var savedDimensions = GravityEntityGeometry.dimensions(failedStandalone);
            var savedPose = failedStandalone.getPose();
            var savedBox = failedStandalone.getBoundingBox();
            int sizeCount = failedStandalone.sizeEvents;
            failedStandalone.throwSize = true;
            boolean propagated = false;
            try { failedStandalone.setPose(Pose.CROUCHING); }
            catch (FixtureFailure expected) { propagated = true; }
            check(propagated && failedStandalone.getPose() == savedPose
                            && GravityEntityGeometry.dimensions(failedStandalone).equals(savedDimensions)
                            && failedStandalone.getBoundingBox().equals(savedBox)
                            && failedStandalone.sizeEvents == sizeCount + 1,
                    "standalone Size failure restores pose/dimensions/proxy without replaying the callback");
            coherent(failedStandalone);
            failedStandalone.discard();
            System.out.println("DEATH_GEOMETRY_CHECKS_PASSED fatal nonfatal outside same-size callback-exception size-exception");
    ...
    private static void coherentGeometry(TestCow cow) {
        check(GravityEntityGeometry.geometryMatchesAxis(cow, runtime(cow).geometryReferenceFrame().up()),"exact body/proxy/axis agree");
    ...
    private static final class TestCow extends Cow {
        int deaths,deathEvents,sizeEvents; boolean throwSize; Runnable callback;
