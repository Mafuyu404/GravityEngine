...
public final class BodyRelativeViewState {
    ...
    public SemanticView semantic(Quaterniond body) {
    ...
    public BodyRelativeViewState bind(Quaterniond body) {
        return !initialized || semantic != null ? this
                : new BodyRelativeViewState(localYaw, localPitch, true, semantic(body));
    }

    public static BodyRelativeViewState fromSemantic(SemanticView semantic, Quaterniond body,
    ...
    public BodyRelativeViewState relativeToBody(Quaterniond body) {
    ...
    public static BodyRelativeViewState activate(
            ...
            Vec3 worldLookBeforeActivation,
            float fallbackYaw,
            float fallbackPitch
    ) {
    ...
    public BodyRelativeViewState rebaseToWorldForward(
        ...
        if (semantic != null) {
            Quaterniond swing = new Quaterniond().rotationTo(
                    new org.joml.Vector3d(semantic.forward().x, semantic.forward().y, semantic.forward().z),
                    new org.joml.Vector3d(desiredWorldForward.x, desiredWorldForward.y, desiredWorldForward.z));
            return fromSemantic(new SemanticView(swing.mul(semantic.worldFromController())), newWorldFromBody, localLook());
        }
        AttitudeSpaceTransform.LocalLookAngles local =
                AttitudeSpaceTransform.worldLookToBodyAngles(
                        newWorldFromBody,
    ...
    private static float wrap(float degrees) {
        return Mth.wrapDegrees(finite(degrees, "degrees"));
    }
