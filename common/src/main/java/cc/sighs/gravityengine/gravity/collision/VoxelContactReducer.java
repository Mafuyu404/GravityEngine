 ...
 *   actual primitive AABBs occupy (never the owner {@code CellPos} alone), so
...
public final class VoxelContactReducer {
    ...
    private static NormalBucket normalBucket(Vec3d normal) {
    ...
    private static boolean isOccludedStaticFace(
        ...
        if (isFullCell(candidateBlock)) {
            CellPos neighborPos = candidateBlock.blockPos().offset(
        ...
        Vec3d surfaceWitness = axisAlignedSurfaceWitness(candidate);
        if (surfaceWitness == null) return false;
        Vec3d outwardSample = surfaceWitness.add(
                new Vec3d(candidate.normal()).multiply(
                        CollisionTolerances.VOXEL_OCCLUSION_PROBE_DISTANCE
                ));
        IdentityHashMap<CollisionContact, Boolean> seen =
                new IdentityHashMap<>();
        CellPos sampleCell = new CellPos(
        ...
        CellPos sampleCoarseCell = new CellPos(
    ...
    private static int axisIndex(Vec3d normal) {
    ...
    private static double faceCoordinate(Aabb3d bounds, Vec3d normal, int axis) {
        return switch (axis) {
    ...
    private static boolean isFullCell(BlockObstacle obstacle) {
        CellPos pos = obstacle.blockPos();
        Aabb3d bounds = obstacle.bounds();
        return within(bounds.minX(), pos.x())
                && within(bounds.minY(), pos.y())
                && within(bounds.minZ(), pos.z())
                && within(bounds.maxX(), pos.x() + 1.0D)
                && within(bounds.maxY(), pos.y() + 1.0D)
                && within(bounds.maxZ(), pos.z() + 1.0D);
    }
    ...
    private static boolean isAxisAligned(Vec3d normal) {
    ...
    private static boolean strictlyContains(Aabb3d bounds, Vec3d point) {
     ...
     * CellPos-backed index.
     *
     * <p>Finite coordinates outside the int-backed cell domain are not malformed
     * geometry. They simply cannot participate in the fine/coarse CellPos index
     ...
     * CellPos-backed spatial-index domain. This is a normal finite-geometry case:
    ...
    private static final class PrimitiveSpatialIndex {
        private final Map<CellPos, List<CollisionContact>> fine = new HashMap<>();
        private final Map<CellPos, List<CollisionContact>> coarse = new HashMap<>();
        ...
        PrimitiveSpatialIndex(
            ...
            for (CollisionContact contact : ordered) {
                 ...
                 * 2. its finite coordinates lie outside the CellPos-backed spatial-index
                 *    domain.
                 *
        ...
        private void registerCells(
                Map<CellPos, List<CollisionContact>> cells,
                CellRange range,
