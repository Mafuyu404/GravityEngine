 ...
 * separate Vanilla fact. Unknown endpoint state retains an existing swim
 * selection but cannot activate one; it never manufactures support from
...
public final class CharacterLocomotionControlPolicyResolver {
    ...
    public static CharacterControlPlan resolve(
        ...
        boolean swimActive = resolvedSwimMode
                && lowGravitySwimMayContinue(state, eligible, groundFacts, frame, config);
    ...
    public static boolean lowGravitySwimEligible(
        ...
        return lowGravitySwimMayContinue(state, eligible, groundFacts, frame, config)
                && groundFacts.terminalExplicitlyUnsupportedAtStepStart();
    }

    /** UNKNOWN is no revocation evidence. Known support and independent gameplay
     * exclusions still end the mode immediately; this never authorizes activation. */
    public static boolean lowGravitySwimMayContinue(
            BodyAttitudePlayerState state, boolean eligible, CharacterGroundFacts groundFacts,
            GravityFrame frame, BodyAttitudeConfigSnapshot config
    ) {
        return eligible
                && !state.fallFlying()
                && !groundFacts
                .gameplayGroundedAtStepStart()
                && !groundFacts.terminalSupportedAtStepStart()
                && frame.strength()
