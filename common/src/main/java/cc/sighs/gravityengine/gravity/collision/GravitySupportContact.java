...
public record GravitySupportContact(
    ...
    public GravitySupportContact(Vec3d normal, Vec3d surfaceVelocity, Vec3d contactPoint,
                                 SupportGeometryKind geometryKind) {
    ...
    public GravitySupportContact {
        ...
        /*
         * Vec3d is mutable. Snapshot ownership therefore requires both
    ...
    public static int blockFaceIndex(Vec3d normal) {
        int axis = TerrainTraversalPolicy.worldAxisDimension(normal);
        return axis < 0 ? -1 : axis * 2
                + (normal.component(axis) > 0 ? 1 : 0);
    }

    /** Finite piece evidence required before a contact can own a reusable plane. */
    public static boolean matchesBlockFace(Aabb3d bounds, Vec3d normal, Vec3d witness) {
        if (blockFaceIndex(normal) < 0) return false;
        Vec3d onFace = blockFaceWitness(bounds, normal, witness);
