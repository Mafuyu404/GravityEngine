...
public interface CollisionScene {
     ...
     * {@link #query(CollisionBody, Vec3d, SweepTimeWindow)}, but support
    ...
    /** Pose-fit at an explicit operation-local obstacle instant, in ticks. */
    default List<CollisionObstacle> queryPoseFit(CollisionBody body, double timeTicks) {
        ...
        return query(body, Vec3d.ZERO, new SweepTimeWindow(timeTicks, 0.0D));
    }
    ...
    /**
     * Immutable engine-owned rigid-obstacle publications captured into this
     * scene.
     *
     * <p>Production captured scenes return the exact list the solver consumed;
     * frozen scenes without rigid publications return an empty list. The list
     * is never a live provider or world handle.</p>
     */
    default List<DynamicCollisionObstacleSnapshot> dynamicObstacles() {
        return List.of();
    }
