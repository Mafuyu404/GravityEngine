 ...
 * <ul>
 *   <li>{@code recoveryMovement} 閳?explicit geometric correction (recovery or selected-face floor snap). It is
 ...
 *   <li>{@code locomotionMovement} 閳?the accepted ordinary/step route. All
...
public record GravityMoveResult(
    ...
    /** Changes endpoint evidence only; trajectory, impact, response constraints and integration frame survive. */
    public GravityMoveResult withEndpoint(Optional<GravitySupportContact> contact, boolean grounded, boolean walkable) {
        return new GravityMoveResult(requestedMovement, resolvedMovement, recoveryMovement, locomotionMovement,
                blockedDown, blockedUp, blockedTangent, supportingContactDuringMove, movementSupportContact,
                grounded, walkable, grounded ? contact.map(GravitySupportContact::faceIdentity).map(SupportFaceIdentity::block)
                        : Optional.empty(), stepHeight, indeterminate, tangentBlockingNormals, supportFollowRise, frame,
                contact, contactVelocityConstraints, indeterminateReason);
    }
    ...
    public GravityMoveResult {
        ...
        List<Vec3d> canonicalTangents =
                new java.util.ArrayList<>();

        Vec3d frameUp =
                frame.orientation().axisY();

        for (Vec3d normal : tangentBlockingNormals) {
            ...
            for (Vec3d existing : canonicalTangents) {
        ...
        /*
         * Vec3d is mutable. A completed move result must own its complete
    ...
    @Override
    public Vec3d locomotionMovement() {
        return locomotionMovement;
    }

    @Override
    public List<Vec3d> tangentBlockingNormals() {
        return List.copyOf(tangentBlockingNormals);
    }
     ...
     * here.</p>
     */
    public Optional<CellPos> authoritativeSupport() {
    ...
    public double fallDistanceVertical() {
        return locomotionMovement().dot(frame.up());
    }
