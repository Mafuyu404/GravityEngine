...
public final class BodyAttitudeComponent {
    ...
    public synchronized void invalidateContinuity() {
        ...
        this.snapshot = new Snapshot(
                /*
                 * Lifecycle invalidation is a mode exit. GE-owned angular
                 * momentum is consumed exactly once here so a later re-entry
                 * cannot inherit stale dynamic continuity through a hidden
                 * stale L_world.
                 */
                current.state().asKinematic(),
                BodyRelativeViewState.uninitialized(),
                null,
                BodyAttitudeDecision.suspended(
    ...
    private static boolean equivalentAuthoritativeState(
        ...
        return equivalentRotation(
                ...
                && equivalentDynamics(current.state(), state)
    ...
    /**
     * Dynamic ownership and durable world-space angular momentum must match
     * exactly (within representation tolerance). Angular velocity is derived
     * and is therefore never compared as an independent authority here.
     */
    private static boolean equivalentDynamics(
            BodyAttitudeState a,
            BodyAttitudeState b
    ) {
        var left = a.angularMomentum();
        var right = b.angularMomentum();
        if (left.isEmpty() || right.isEmpty()) {
            return left.isEmpty() && right.isEmpty();
        }
        return left.get().inertia().equals(right.get().inertia())
                && equivalentVec(
                left.get().angularMomentumWorld(),
                right.get().angularMomentumWorld()
        );
    }
}
